...
- 直接指导人：杨欣怡
- 代码审核人：杨欣怡、陈尧
- 你负责的角色：资源生成智能体与学习资料页面
- 契约状态：`frozen_v6`

### 允许修改的代码范围

- `services/agents/resource-generation/**`
- `services/resources/**`
...
17. `docs/12_前端页面接口依赖.md`
18. `api/internal_agent_interfaces.yaml`
19. `contracts/jsonschema/agents/resource_input.schema.json`
20. `contracts/jsonschema/agents/resource_output.schema.json`
...
- `resource_generation_agent`：实现负责人
...
1. 阶段0：学习资源输入输出契约和Mock
2. 阶段1：用Mock完成资源列表、详情、引用与进度页面
3. 阶段2：修复引用展示问题
4. 阶段3：接入画像和路径上下文
5. 阶段4：实现资源生成智能体并联调
6. 阶段5：实现审核驳回后的重新生成
...
```text
你正在完成挑战杯项目中“资源生成智能体与学习资料页面”相关任务。必须遵守以下规则：
1. 只修改任务卡允许的目录：services/agents/resource-generation/**, services/resources/**, apps/web/src/features/resources/**, tests/resources/**。

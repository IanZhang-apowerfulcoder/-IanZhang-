...
19. `contracts/jsonschema/agents/assessment_input.schema.json`
20. `contracts/jsonschema/agents/assessment_output.schema.json`
...
1. 阶段0：学习测评输入输出契约和Mock
2. 阶段1：用Mock完成题目、作答和结果页面
3. 阶段2：整理题库与引用展示
4. 阶段3：接入画像和目标知识节点
5. 阶段4：实现测评生成、提交和评分联调
6. 阶段5：实现驳回后重新出题
7. 阶段6：边界测试和修复
8. 阶段7：演示配合
...
```text
你正在完成挑战杯项目中“测评智能体与练习页面”相关任务。必须遵守以下规则：
1. 只修改任务卡允许的目录：services/agents/assessment/**, services/assessment/**, apps/web/src/features/assessment/**, tests/assessment/**。

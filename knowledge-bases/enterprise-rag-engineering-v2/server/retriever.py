...
class DomainRetriever:
    """统一可信知识层：核心知识与第二专业 AI 知识包均默认可检索。"""
    ...
    def __init__(self, base_dir: Path | None = None) -> None:
        ...
        trusted = [json.loads(line) for line in (self.base / "data/rag_chunks.jsonl").read_text(encoding="utf-8").splitlines() if line.strip()]
        for c in trusted:
            c.setdefault("content_status", "trusted")
            c.setdefault("review_status", "candidate_validated")
            c.setdefault("origin_package", "enterprise-rag-engineering-domain-pack-v2.0")
        candidate = []
        self.trusted_chunks = trusted
        self.candidate_chunks = candidate
        self.chunks = trusted
        self.evidence_map = {c["evidence_id"]: c for c in self.chunks}
        self.edges = json.loads((self.base / "data/knowledge_edges.json").read_text(encoding="utf-8"))
        self.docs = []
    ...
    def _allowed(self, chunk: dict[str, Any], filters: dict[str, Any]) -> bool:
        filters = filters or {}
        if filters.get("content_status") and chunk.get("content_status") not in set(filters["content_status"]):
            return False
    ...
    def search(self, query: str, top_k: int = 5, filters: dict[str, Any] | None = None) -> list[dict[str, Any]]:
        ...
        for chunk, tf, dl in self.docs:
            ...
            exact_terms = [node["name"], *node.get("aliases", []), *chunk.get("aliases", [])]
        ...
        return [{
            ...
            "knowledge_base_version_id": c["knowledge_base_version_id"],
            ...
            "content_status": c.get("content_status", "trusted"),
            "review_status": c.get("review_status"),
            "origin_package": c.get("origin_package"),
            "external_subtopic_id": c.get("external_subtopic_id")
    ...
    def compatible_response(self, request: dict[str, Any]) -> dict[str, Any]:
        ...
        safe_filters = dict(request.get("filters") or {})
        hits = self.search(request["query_text"], int(request["top_k"]), safe_filters)
        return {

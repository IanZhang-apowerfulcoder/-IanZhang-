...
app = FastAPI(title="Enterprise RAG Engineering Domain Pack API", version="1.0.0")
...
def health() -> dict[str, Any]:
    return {"status":"ok","version":retriever.domain["version"],"chunk_count":len(retriever.chunks)}
...
@app.post("/api/v1/retrieval/search:extended")
def extended_search(body: RetrievalRequest) -> dict[str, Any]:
    ...
    hits = retriever.search(body.query_text, body.top_k, body.filters)
    return {
        "retrieval_request_id": body.retrieval_request_id,
        "knowledge_base_version_id": body.knowledge_base_version_id,
        "strategy": "bm25_char_ngram_metadata_baseline",
...
def list_tasks(knowledge_node_id: str | None = None) -> list[dict[str, Any]]:
    return tasks if not knowledge_node_id else [t for t in tasks if t["knowledge_node_id"] == knowledge_node_id]
